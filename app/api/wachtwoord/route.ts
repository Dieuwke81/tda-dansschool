...
export async function POST(req: NextRequest) {
  ...
  if (!username) {
    return NextResponse.json(
      { success: false, error: "Gebruiker ontbreekt in sessie" },
      { status: 401 }
    );
  }
  ...
  if (newPassword !== confirmPassword) {
    return NextResponse.json(
      { success: false, error: "Nieuwe wachtwoorden komen niet overeen" },
      { status: 400 }
    );
  }
  ...
  if (newPassword.length < 8) {
    return NextResponse.json(
      { success: false, error: "Wachtwoord moet minimaal 8 tekens zijn" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Kon Google Sheet niet ophalen" },
      { status: 500 }
    );
  }
  ...
  const iMustChange = idx(map, "moet_wachtwoord_wijzigen");
  ...
  // we checken deze ook even zodat je meteen weet of je sheet klopt
  if (iMustChange === -1) {
    return NextResponse.json(
      { success: false, error: "Sheet kolom mist: moet_wachtwoord_wijzigen" },
      { status: 500 }
    );
  }
  ...
  // ✅ gebruik norm() zodat Alexandra Dogge altijd matcht (ook met rare spaties/case)
  const row = rows
    .map(parseCsvLine)
    .find((c) => norm(c[iUsername]) === norm(username));

  if (!row) {
    return NextResponse.json(
      { success: false, error: "Account niet gevonden" },
      { status: 404 }
    );
  }
  ...
  try {
    const r = await fetch(writeUrl, {
      ...
      body: JSON.stringify({
        secret: writeSecret,
        username,                 // username om de rij te vinden
        newHash,                  // nieuwe bcrypt hash
        moet_wachtwoord_wijzigen: "nee", // ✅ matcht jouw sheet kolom (zet op "nee")
      }),
    });
    ...
    return NextResponse.json(
      { success: false, error: "Kon wachtwoord niet opslaan" },
      { status: 500 }
    );
  }
